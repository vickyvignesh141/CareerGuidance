...
def ask_student_questions():
    responses = {}
    print("🧑‍🎓 Please answer these questions (press Enter if not sure):\n")
    for q in questions:
        a = input(f"{q}\n➡ ").strip()
        responses[q] = a
...
def generate_daily_schedule(goal, hours_per_day: float, topics: List[str]):
    if not topics:
        return {"goal": goal, "hours_per_day": hours_per_day, "schedule": {}, "note": "No topics to study — you already know them all!"}
...
def generate_weekly_schedule(goal, weekly_hours: float, topics: List[str]):
    if not topics:
        return {"goal": goal, "weekly_hours": weekly_hours, "schedule": {}, "note": "No topics to study — you already know them all!"}
...
def run_agent_flow():
    ...
    print("⏳ Generating career guidance (this may take a few seconds)...")
    ...
    if len(careers) >= 3:
        print("\n🎯 Here are your recommended careers:")
    ...
    print(f"\n📘 Suggested core topics for {top_career}: {topics}")
    ...
    if not missing:
        print("\n✅ You already know the suggested topics! Fetching an advanced plan...")
        ...
        print("\n🚀 Building learning schedules focused on missing topics:", missing)

        # Ask time availability
        while True:
            try:
                daily_hours = float(input("⏱ Hours you can study daily (e.g., 2):\n➡ ").strip())
                break
            except:
        ...
        while True:
            try:
                weekly_hours = float(input("⏱ Total weekly study hours (e.g., 10):\n➡ ").strip())
                break
            except:
                print("Please enter a valid number.")
    ...
    print("\n🎉 All done! Run the agent again anytime.")
